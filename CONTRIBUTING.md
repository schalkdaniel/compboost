# Contributing to `compboost` development

We, the authors of the `compboost` R package, use the same guide as is used for
...
The goal of this guide is to help you get up and contributing to `compboost` as
...
1.  The easiest way to include **data** is to use `dput()` to generate the R
    code to recreate it. For example, to recreate the `mtcars` dataset in R,
    I'd perform the following steps:

       1. Run `dput(mtcars)` in R
       1. Copy the output
       1. In my reproducible script, type `mtcars <- ` then paste.
...
To contribute a change to `compboost`, you follow these steps:
