...
spline.factory.fare = PSplineBlearner$new(data_source = data.source.fare, 
...
# Define spline factory:
spline.factory.age = PSplineBlearner$new(data_source = data.source.age, 
...
An idiosyncraty of splines here is that they initialize sparse matrices by default
...
for (class in classes.sex) {
  ...
  data.sex.list[["factory"]][[data.name]] = PolynomialBlearner$new(
...
for (class in classes.pclass) {
  ...
  data.pclass.list[["factory"]][[data.name]] = PolynomialBlearner$new(
...
```{r}
used.optimizer = CoordinateDescent$new()
```
