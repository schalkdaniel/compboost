...
High-level plotting functions are available to visualize the risk (`plotRisk()`), partial feature effects (`plotBaselearner()` for univariate and `plotTensor()` for bivariate base learner), base learner selection traces (`plotBaselearnerTraces()`), or the feature importance (`plotFeatureImportance()`)
```{r}
library(ggplot2)
library(patchwork)
...
(plotRisk(cboost) |
 ...
 plotFeatureImportance(cboost)) /
  (plotBaselearner(cboost, "Fare_spline") |
   plotBaselearner(cboost, "Age_spline") |
   plotBaselearner(cboost, "Sex_categorical")) &
  theme_minimal()
```
