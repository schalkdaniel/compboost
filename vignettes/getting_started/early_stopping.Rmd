...
- the loss $L$ that is used for stopping: $$\mathcal{R}_{\text{emp}}^{[m]} = \frac{1}{n}\sum_{i=1}^n L\left(y^{(i)}, f^{[m]}(x^{(i)})\rigth)$$
...
cboost$addLogger(logger = LoggerOobRisk, use_as_stopper = TRUE, logger_id = "oob",
  used_loss = LossBinomial$new(), eps_for_break = 0, oob_data = oob_data, oob_response = oob_response)
...
Taking a look at the logger data tells us that we stopped after the first difference is bigger than zero (the oob risk of the current iteration is bigger than the previous one):
```{r}
tail(cboost$getLoggerData())
diff(tail(cboost$getLoggerData()$oob))
```

```{r}
library(ggplot2)
ggplot(data = cboost$getLoggerData(), aes(x = `_iterations`, y = oob)) + geom_line()
```

**Note:** It could happen that the model's oob behavior increases locally for a view iterations and then starts to decrease again. To capture this we would need a "patience" parameter which waits for, lets say, 10 iterations and breaks just if all 10 iterations fulfill the criteria.
