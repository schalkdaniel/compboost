...
\]
Within that equation we do not transpose $Y$ and therefore can take advanatage of the sparse structure. In `C++` this looks as follows (in `baselearner.cpp` function `PSplineBlearner::predict()`):
```cpp
