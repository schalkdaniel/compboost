...
test_that("factory list works", {
  ...
  expect_silent({ linear.factory.hp = PolynomialBlearner$new(data.source.hp, data.target1, 1, FALSE) })
  expect_silent({ linear.factory.wt = PolynomialBlearner$new(data.source.wt, data.target2, 1, FALSE) })
  expect_silent({ quadratic.factory.hp = PolynomialBlearner$new(data.source.hp, data.target3, 2, FALSE) })
  expect_silent({ pow5.factory.hp = PolynomialBlearner$new(data.source.hp, data.target4, 5, FALSE) })
